...
class blah {
	...
	function setPassword($user_id, $password) {
		// just in case...
		if ($password) {
			$this->dbQuery("update ".USERS_TABLE." set password = password('".addSlashes($password)."') where id = '".addSlashes($user_id)."' limit 1");
		}
	}
	...
	function loginUser($email, $password, $remember = 0) {
		// load user
		if ($user = $this->dbLoadSingle("select * from ".USERS_TABLE." where (email = '".addSlashes($email)."' or name = '".addSlashes($email)."') and password = password('".addSlashes($password)."') and is_verified = 'Y' limit 1")) {
	...
	function onSignup() {
		if ($_POST["submit"]) {
			...
			if (!$error) {
				...
				// send email
				mail($user["email"], SITE_TITLE." Account Activation Link", 
					"Hi ".$user["name"]."!\n\n".
	...
	// NOTE OpenCrap does not handle semithreading currently
	...
	function xmlTopics() {
		...
		$time = $_GET["time"];
		if(isset($time)) {
			$extra = " and (when_last_comment > '$time' or comment_count = 0) ";
		}
		else {
			$extra = "";
		}

		$data = $this->loadTopics("where location='frontpage'".$extra."order by when_published desc limit $offset,$limit");
		for($i = 0; $i < count($data); $i++) {
			$unread = $this->countNewTopicComments($data[$i]);
			$data[$i]["unread_count"] = is_numeric($unread) ? $unread : 0;
		}
		$this->dumpXml($data, "topic");
	}
	
	function xmlLastRead() {
		$email = $_GET["email"];
		$password = $_GET["password"];
		$this->loginUser($email, $password);
		$thread_id = $_GET["thread_id"];
		$time = $_GET["time"];
		$this->setLastRead($thread_id, $time);
	}
	...
	function xmlComments() {
		...
		// Strip out email if user has indicated to do that and avatar
		for($i = 0; $i < count($data); $i++) {
			$data[$i]["author_avatar"] = "";
			if($data[$i]["author_id"]) {
				if($author = $this->loadUser($data[$i]["author_id"])) {
					$data[$i]["author_email"] = ($author["show_email"] == "Y" ? $author["email"] : "");
					$data[$i]["author_avatar"] = $author["avatar_filename"];
				}
			}
		}
		...
		if(isset($email) && isset($password)) {
			$this->loginUser($email, $password);
			$this->setLastRead($thread_id, $this->unix2datetime());
		}
		
		$this->dumpXml($data, "comment");
	}
	
	// Stripped down from onPostComment()
	function xmlPost() {
		$topic_id = $_POST["topic_id"];
		$thread_id = $_POST["thread_id"];
		$comment = $_POST["comment"];
		...
		$topic = $this->loadTopic($topic_id) or die("LOSE..");
		$thread = $this->loadThread($thread_id) or die("LOSE..");
		
		$comment = $this->arrayTrim($this->arrayForceKeys($comment, array("author_name", "author_email", "author_url", "body")));

		// Registered user
		if($this->loginUser($comment[author_email], $password)) {
			$comment["author_name"] = $_SESSION[USER_SESSIONVAR]["name"];
			$comment["author_email"] = $_SESSION[USER_SESSIONVAR]["email"];
			$comment["author_url"] = $_SESSION[USER_SESSIONVAR]["url"];
			$comment["author_id"] = $_SESSION[USER_SESSIONVAR]["id"];
		...
		// Get a number. This gets overwritten later in insertComment()
		$data = $this->dbLoadSingle("select max(num) as maxnum from ".COMMENTS_TABLE." where thread_id = '".addSlashes($thread["id"])."'");
		$comment["num"] = $data["maxnum"] + 1;
